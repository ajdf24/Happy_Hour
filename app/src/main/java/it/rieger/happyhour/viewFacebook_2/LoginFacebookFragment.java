...
public class LoginFacebookFragment extends Fragment {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mtracker = (AccessTokenTracker) (oldAccessToken, currentAccessToken) -> {
            Log.v("AccessTokenTracker", "oldAccessToken=" + oldAccessToken + "||""CurrentAccessToken" + currentAccessToken)
            homeFragment(currentAccessToken);
        };

        mprofileTracker = (ProfileTracker) (oldProfile, currentProfile) -> {
            Log.v("Session Tracker", "oldProfile" + oldProfile + "||" + "currentProfile" + currentProfile)
            homeFragement(currentProfile);
        };
    ...
    private void homeFragment(Profile profile) {
        if (profile != null) {
            ...
            LoginFacebookFragment lff = new LoginFacebookFragment();
            lff.setArguments(mBundle);

            FragmentManager fragmentManager = getActivity().getSupportFragmentManager();
            FragmentTransaction fragmentTransaction = fragmentManager.beginTransaction();
            fragmentTransaction.replace(R.id.mainContainer, LoginFacebookFragment());
